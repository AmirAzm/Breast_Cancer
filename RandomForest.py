...
c = 0
########################::|:: TRAIN_TEST_SPLIT ::|:: #######################################
result_table = pd.DataFrame(columns=['round','data_source', '#train_sample','#test_sample','classifier','Correct','#N-estimators','criterion','TP','FP','TN','FN','P','N','precision','recall','f-1','ROC_AUC','accuracy'])
for x,name,x_Test in zip(X,['Mean','Standard_error','Max_value'],X_test):
    ...
    for cri in ['gini','entropy']:
        for esti in [100,300,500,800,1000]:
            #x_train, x_test, y_train, y_test = train_test_split(x, Y, test_size=i, random_state=0)
            c+=1
            clf = RandomForestClassifier(criterion=cri,n_estimators=esti)
            clf.fit(x,Y)
...
############################################################################################
########################::|::K-FOLD::|::#######################################################
# result_table = pd.DataFrame(columns=['round','data_source', '#train_sample','#test_sample','#Fold','classifier','Correct','#N-estimators','criterion','TP','FP','TN','FN','P','N','precision','recall','f-1','ROC_AUC','accuracy'])
# for x,name,x_Test in zip(X,['Mean','Standard_error','Max_value'],X_test):
...
#                     c+=1
#                     clf = RandomForestClassifier(n_estimators=esti,criterion=cri)
...
#             'roc_auc':'roc_auc',
#            'accuracy':'accuracy'}
# result_table = pd.DataFrame(columns=['round','data_source','#Fold','classifier','#N-estimators','criterion','TP','FP','TN','FN','P','N','precision','recall','f-1','ROC_AUC','accuracy'])
...
#############################################################################################
########################::|:: SAVE RESULTS ::|::#############################################
result_table.to_excel('TESTRe/0.xlsx',sheet_name='Result',index=False)